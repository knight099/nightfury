...
- **AI:** Gemini 2.5 Flash for vision, gated behind motion detection *and* a local YOLOv8n ONNX pass, with circuit breaker + confidence filtering on responses. Digest summaries run a single Gemini *text* call over event metadata only — no raw video ever reaches Gemini beyond individual motion-gated frames.

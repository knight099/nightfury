...
- **Cost engineering:** motion detection gates all Gemini calls; a local YOLO26n ONNX pass (end-to-end/NMS-free export) further gates/short-circuits Gemini for simple person/vehicle/animal/intrusion detections; digest summaries use one Gemini text call over event metadata instead of per-event summarization.
- **Pose/sequence tracking:** an optional per-camera step-sequence engine (local YOLO26n-pose + IoU/ByteTrack tracker + state machine) flags skipped or stalled steps in a defined procedure without any additional Gemini calls.

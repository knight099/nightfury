...
Vision Models"*, CVPR 2025 — <https://seeingmorewithless.github.io/>

The PDF is deliberately not committed (2.9MB binary; root-level `*.pdf` is
gitignored). Every number this spec relies on is quoted inline below, so the
paper is not needed to review or act on it.
